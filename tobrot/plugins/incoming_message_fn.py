...
async def incoming_message_f(client, message):
    """/leech command"""
    i_m_sefg = await message.reply_text("patience 🛃 while ....processing" , quote=True)
    is_zip = False
    is_unzip = False
    is_unrar = False
    is_untar = False
    ...
    if dl_url is not None:
        await i_m_sefg.edit_text("patience 🛃 while... i extract links❗️")
        ...
        await i_m_sefg.edit_text("🛃 let me try ..📤 to download")
...
async def g_yt_playlist(client, message):
    ...
    if 'youtube.com/playlist' in message.reply_to_message.text:
        i_m_sefg = await message.reply_text("🛃 Downloading...you should wait⏳❗️", quote=True)
