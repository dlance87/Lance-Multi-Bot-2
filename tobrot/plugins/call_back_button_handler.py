...
async def button(bot, update: CallbackQuery):
    ...
    if "|" in cb_data:
        ...
        if cb_data.startswith("cancel"):
            if len(cb_data) > 1:
                i_m_s_e_g = await update.message.reply_text("🛃..patience..let me check..🔎❗️", quote=True)
                ...
                try:
                    ...
                    if os.path.exists(file_name):
                        if os.path.isdir(file_name):
                            ...
                            os.remove(file_name)
                    await i_m_s_e_g.edit_text(f"🛃Operation 🚫 Cancelled by <a href='tg://user?id={update.from_user.id}'>{update.from_user.first_name}</a>")
                ...
                if len(g_del_list) != 0:
                    for f in g_del_list:
                        if os.path.isfile(f):
                            ...
                            shutil.rmtree(f)
                    await update.message.edit_text(f"💁‍♂️Deleted {len(g_del_list)} objects.")

                else:
                    await update.message.edit_text("🙆‍♂️..Nothing else..🙌 to clear 😥")
            else:
                await update.message.edit_text("You are not allowed to do that 🤭❗️")
