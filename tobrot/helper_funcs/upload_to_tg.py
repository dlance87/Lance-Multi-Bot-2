    ...
    if os.path.isdir(local_file_name):
        ...
        if os.path.getsize(local_file_name) > TG_MAX_FILE_SIZE:
            ...
            await i_m_s_g.edit_text(
                ...
                f"<code>{ba_se_file_name}</code> splitted  ✂️ into {number_of_files} files.\n"
...
async def upload_to_gdrive(file_upload, message, messa_ge, g_id):
    ...
    del_it = await message.edit_text("patience 🛃 while uploading .. to ☁️ Cloud❗")
    ...
    if os.path.isfile(file_upload):
        ...
        await asyncio.sleep(EDIT_SLEEP_TIME_OUT)
        await messa_ge.reply_text(f" File 📂: {file_upload} has been Uploaded🧑‍💻 successfully to your ☁️ Cloud <a href='tg://user?id={g_id}'>😁</a>\n📀 Size: {gjay}", reply_markup=button_markup)
...
async def upload_single_file(message, local_file_name, caption_str, from_user, edit_media):
    ...
    if UPLOAD_AS_DOC.upper() == 'TRUE':
        ...
        if not edit_media:
            message_for_progress_display = await message.reply_text("patience 🛃 let.. me start uploading of {}".format(os.path.basename(local_file_name)))
        ...
        try:
            ...
            if not edit_media:
                message_for_progress_display = await message.reply_text(
                    "🛃 patience .. 🧑‍💻 starting upload of {}".format(os.path.basename(local_file_name))
                )
